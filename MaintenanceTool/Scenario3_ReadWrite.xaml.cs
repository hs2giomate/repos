...
namespace MaintenanceToolECSBOX
{
    ...
    public sealed partial class Scenario3_ReadWrite : Page, IDisposable
    {
        ...
        private Boolean magiSent = false;
        ...
        {
            if (EventHandlerForDevice.Current.Device.WriteTimeout.TotalMilliseconds==0)
            {
        ...
        {
            if (EventHandlerForDevice.Current.Device.ReadTimeout.TotalMilliseconds==0)
            {
                EventHandlerForDevice.Current.Device.ReadTimeout = new System.TimeSpan(49,17,2,47,295);
            }
        ...
        async private void ReadECSFeedBack()
        {
            if (EventHandlerForDevice.Current.IsDeviceConnected)
            {
                try
                {
                    rootPage.NotifyUser("Reading ECS BOX...", NotifyType.StatusMessage);
                ...
                if ((readText != null)&&(magiSent))
                {
                    if (readText.Contains("CPU Serial"))
                    {
        ...
        private async void WriteButton_Click(object sender, RoutedEventArgs e)
        {
            if (EventHandlerForDevice.Current.IsDeviceConnected)
            {
                ...
                if (magiSent&(aTimer==null))
                {
                    while (IsPerformingWrite())
                    {
                        ;
                    }
                    ReadECSFeedBack();
        ...
        private async Task ReadAsync(CancellationToken cancellationToken)
        {
            ...
            if (bytesRead > 0)
            {
                ...
                ReadBytesTextBlock.Text += readText;
